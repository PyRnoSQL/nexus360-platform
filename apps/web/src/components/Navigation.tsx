      ...
      <nav className={`fixed left-0 top-0 h-full bg-navy-900 border-r border-navy-700 flex flex-col transition-all duration-300 z-50 hidden md:flex ${isCollapsed ? 'w-20' : 'w-64'}`}>
        <div className={`p-6 border-b border-navy-700 flex ${isCollapsed ? 'justify-center' : 'justify-between'} items-center relative`}>
          ...
          <button 
            ...
            className="p-1 hover:bg-navy-800 rounded-lg absolute -right-3 top-20 bg-navy-700 border border-navy-600 rounded-full hidden md:block"
          >
            ...
            {!isCollapsed ? (
              ...
              <div className="flex justify-center">
                <div className="w-8 h-8 bg-gold-500/20 rounded-full flex items-center justify-center">
                  <span className="text-gold-500 text-xs font-bold">{userName.charAt(0)}</span>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="flex-1 py-4 px-3 space-y-2 overflow-y-auto">
          {visibleItems.map((item) => {
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg ${active ? 'bg-gold-500/10 text-gold-500 border border-gold-500/20' : 'text-gray-400 hover:bg-navy-800 hover:text-white'}`}
              >
          ...
          {canAccessForms && (
            <Link 
              to="/forms" 
              onClick={toggleMobileMenu} 
              className={`flex items-center gap-3 px-3 py-2 rounded-lg ${location.pathname === '/forms' ? 'bg-gold-500/10 text-gold-500 border border-gold-500/20' : 'text-gray-400 hover:bg-navy-800 hover:text-white'}`}
            >
