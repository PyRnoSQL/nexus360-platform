...
export const FinanceDashboard = ({ user }) => {
  const { t } = useTranslation();
  const userRole = user?.role;
  const dashboardKey = 'finance';
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white mb-2">{t('dashboard.finance')}</h1>
        <p className="text-gray-400">Suivi budgétaire et conformité financière</p>
      </div>
      <DashboardEmbed 
